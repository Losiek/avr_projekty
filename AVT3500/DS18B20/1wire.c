...
//Odczyt bajtu z urz¹dzenia 1-Wire.(2)
void read_1w(void) {
...
uint8_t reset_1w(void) {
	...
	if(PIND&(1<<wire))  {
		zp=0;								//zapamiêtaj fakt wykrycia urz¹dzenia
	}
	...
	if(PIND&(1<<wire)) {
		zp=1;								//jeœli pozostaje w stanie niskim, to oznacza zwarcie
	}										
	
	if(zp) {
		return 1;							//brak pod³¹czonych urz¹dzeñ	
	} else {								
		return 0;							//pod³¹czone urz¹dzenie
	}										
}
